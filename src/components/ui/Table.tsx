...
export function Table<T>({ 
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-md border border-slate-200 dark:border-zinc-800 shadow-sm overflow-hidden flex flex-col">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-zinc-800">
            {isLoading ? (
              ...
              displayedData.map((item, rowIdx) => (
                <tr 
                  key={(item as any)._rowId ?? (item as any).id ?? rowIdx} 
        ...
        <div className="px-4 sm:px-6 py-4 border-t border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-800/30 flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-0">
          ...
          <div className="flex items-center justify-center gap-1 sm:gap-2 w-full sm:w-auto">
            <button
              ...
              className="px-3 sm:px-4 py-2 rounded-sm border border-slate-200 dark:border-zinc-700 text-xs sm:text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-zinc-700 disabled:opacity-50 disabled:!cursor-not-allowed transition-colors"
            >
              Anterior
            </button>
